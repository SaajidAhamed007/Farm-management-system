  ...
  return (
    <div className="min-h-screen bg-gray-300 p-8">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-responsive-4xl font-bold text-green-700 mb-2">Village Issues</h1>
          <p className="text-responsive-lg text-gray-600">Manage and resolve village issues</p>
        </div>

        {/* Filters and Sorting */}
        <div className="bg-gray-200 rounded-xl shadow-lg p-6 mb-8">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {/* Category Filter */}
            <div>
              <label className="block text-responsive-lg font-medium text-gray-700 mb-2">Category</label>
              <select
                value={filterCategory}
                onChange={(e) => setFilterCategory(e.target.value)}
                className="w-full p-3 border border-green-600 bg-gray-100 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent text-responsive-base"
              >
            ...
            {/* Status Filter */}
            <div>
              <label className="block text-responsive-lg font-medium text-gray-700 mb-2">Status</label>
              <select
                value={filterStatus}
            ...
            {/* Sort By */}
            <div>
              <label className="block text-responsive-lg font-medium text-gray-700 mb-2">Sort By</label>
              <select
                value={sortBy}
            ...
            {/* Sort Order */}
            <div>
              <label className="block text-responsive-lg font-medium text-gray-700 mb-2">Order</label>
              <select
                value={sortOrder}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {filteredAndSortedIssues.map((issue) => (
            <div
              key={issue.id}
              className="bg-gray-200/60 rounded-xl shadow-lg p-6 cursor-pointer hover:shadow-xl transition-all duration-200 border-l-4 border-green-500"
              onClick={() => handleIssueClick(issue)}
            >
              {/* Header */}
              <div className="flex justify-between items-start mb-4">
                <div className="flex-1">
                  <h3 className="text-responsive-xl font-bold text-gray-800 mb-2">{issue.title}</h3>
                  <div className="flex flex-wrap gap-2 mb-2">
                    <span className={`px-3 py-1 rounded-full text-responsive-sm font-medium ${getStatusColor(issue.status)}`}>
                      {issue.status}
                    </span>
                    <span className="px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-responsive-sm font-medium">
                      {issue.category}
                    </span>
                  </div>
                </div>
              ...
              {/* Description */}
              <p className="text-responsive-base text-gray-600 mb-4">{issue.description}</p>

              {/* Details */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-responsive-sm">
                <div>
                  <p className="text-gray-500">Reported by:</p>
                  <p className="font-medium text-gray-800">{issue.reportedBy}</p>
                </div>
                <div>
                  <p className="text-gray-500">Location:</p>
                  <p className="font-medium text-gray-800">{issue.location}</p>
                </div>
              </div>

              {/* Action indicator */}
              <div className="mt-4 pt-4 border-t border-gray-200">
                <p className="text-green-600 font-medium text-responsive-sm">
        ...
        {/* Summary Stats */}
        <div className="mt-8 bg-gray-200/60 rounded-xl shadow-lg p-6">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-responsive-2xl font-bold text-gray-800">Summary</h3>
            <div className="w-48">
              <label className="block text-responsive-sm font-medium text-gray-700 mb-2">Time Period</label>
              <select
                value={timePeriod}
                onChange={(e) => setTimePeriod(e.target.value)}
                className="w-full p-3 border border-gray-300 bg-gray-100/80 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent text-responsive-base"
              >
